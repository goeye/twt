...
5. **隐藏功能菜单**：切换至备注模式后，输入框上方的Copilot推荐回复与远程协助功能将隐藏；表情、快捷回复、附件、图片、文本润色及AI翻译功能可正常使用。此外，备注模式下开启边写边译，内容将以备注消息形式发送。
6. **Copilot 推荐消息备注模式排除**：若在切换至备注模式前已开启 Copilot 推荐回复或者开启了自动推荐功能，其发送内容仍为普通消息，不会作为备注消息发送。
7. **文本润色时无法开启备注模式**：文本润色中（帮我润色、丰富内容、精简内容）无法点击模式切换按钮，禁止点击
