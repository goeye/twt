...
2. **「内部备注」标签**：气泡内显示「内部备注」文字标签
3. **位置**：备注消息由客服发送，在消息时间线中的位置与普通客服消息一致
4. **访客端不展示**：备注消息不在访客端渲染
5. **操作菜单**：备注气泡的 hover 操作菜单仅显示复制，其他消息操作（如：撤回、回复、翻译等）不可用
6. **AI 忽略**：备注回复会被AI忽略，不作为上下文传入AI
7. **风控忽略**：备注回复不传递给风控检测
...
**功能描述**：回复模式下发送图片、视频、文件等媒体内容时，消息以专属卡片样式展示，与纯文字消息可区分。

**需求描述（功能规则）**：
1. **图片**：以缩略图形式内嵌在消息气泡中展示
2. **文件/附件**：以文件卡片形式展示，包含文件图标、文件名
3. **背景色**：媒体消息气泡使用与普通客服消息一致的专属背景色，与备注消息气泡背景色不同
4. **混合内容**：文字与媒体内容可同时发送，文字显示在媒体内容上方
5. **仅文字时**：无附件时，发送按钮在输入框为空时禁用；有附件时，即使输入框为空也可发送
