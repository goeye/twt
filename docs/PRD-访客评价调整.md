...
本次调整将会话评价从「会话结束后出现」改为「输入框上方快捷入口按钮」，hover 后显示评价卡片，支持会话活跃期间随时评价、反复修改和取消。
...
| US-07 | 客服 | 我希望在聊天记录中看到会话评价的系统消息 | 消息列表中显示评价相关的系统消息 | P0 |
| US-08 | 客服 | 我希望在会话记录档案中查看会话评价 | 档案列表和筛选器中显示「会话评价」字段 | P0 |
| US-09 | 客服 | 我希望在会话记录档案的聊天抽屉中看到评价系统消息 | 抽屉消息列表中显示评价相关的系统消息 | P0 |
...
2. 在筛选器中看到「会话评价」下拉选项
3. 在表格列头中看到「会话评价」列
4. 点击会话行的「操作」按钮，打开聊天抽屉
5. 抽屉中的消息列表显示评价相关的系统消息

**需求描述（功能规则）**：
1. **筛选器标签**：「会话评价」（替换原「访客评价」）
2. **筛选器选项**：全部、满意、一般、不满意、未评价
3. **表格列头**：「会话评价」（替换原「访客评价」）
4. **表格单元格值**：满意、一般、不满意、—（未评价）
5. **聊天抽屉消息**：显示评价相关的系统消息，样式与活跃会话一致（居中显示，无头像）
...
| 会话信息 | 会话评价 | 枚举（满意/一般/不满意/暂无评价） | 客服端会话信息面板展示的评价字段 |
...
| 查看会话信息面板中的会话评价 | 不可见 | 可见 | 可见 |
| 查看聊天消息列表中的评价系统消息 | 可见（显示"你"） | 可见（显示"访客"） | 可见（显示"访客"） |
| 查看会话记录档案中的会话评价 | 不可见 | 可见 | 可见 |
| 查看会话记录档案聊天抽屉中的评价系统消息 | 不可见 | 可见 | 可见 |
...
| 客服端会话信息面板 | 数据同步 | 会话评价字段实时显示访客的当前评价结果 |
| 客服端聊天消息列表 | 消息同步 | 会话评价操作产生的系统消息实时显示在消息列表中 |
| 客服端会话记录档案 | 数据展示 | 档案列表和筛选器显示会话评价字段，聊天抽屉显示评价系统消息 |
