...
| ID | 角色 | 用户故事 | 验收标准 | 优先级 |
|----|------|---------|----------|--------|
| US-01 | 管理员 | 我希望在设置中添加公司 Gmail 邮箱，以便团队能在 TWT Chat 中统一收发邮件 | 通过 Gmail OAuth 授权后，邮箱出现在已连接列表中，Toast 提示"邮箱连接成功" | P0 |
...
**用户场景**：管理员需要为团队接入公司邮箱，使客服能在 TWT Chat 中处理邮件咨询。

**前置条件**：
1. 用户具有 Email 渠道的"管理"权限

**交互流程**：
1. 进入 Email 页面，页面标题为"Email"，下方描述文案为"在 TWT Chat 中统一收发其他邮箱服务商的邮件，所有邮件都会以会话形式呈现，便于统一管理"
...
3. **消息操作**：
   - Email 会话中的消息不显示操作工具栏（回复、复制、翻译、撤回）
...
4. **附件规则**：
   ...
   - 图片文件：内嵌到编辑器正文中
   - 非图片文件：以附件卡片形式显示在编辑器下方，展示文件图标、文件名、大小和移除按钮
...
**功能描述**：会话记录档案中增加"来源渠道"维度，支持按 Web/Email 筛选和展示。

**用户场景**：管理员在归档列表中查看和筛选邮件会话的历史记录。
...
3. **操作菜单差异**：
   - 邮件会话在排队中状态：管理员可"分配会话"，普通客服可"分配给我"，均可"查看会话"
   - 邮件会话在非排队状态：可"查看会话"
   - 会话详情抽屉中，邮件会话在非排队状态下操作按钮文案变为"加入会话"
...
| 档案模块 | 筛选和操作差异 | 新增来源渠道筛选列，邮件会话有不同的操作菜单 |
| 权限模块 | 权限控制 | Email 渠道设置页受独立权限项控制 |
