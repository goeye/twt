...
*   不支持边写边译、文本润色和远程协助功能（注：此处指编辑器内的实时翻译面板，消息级别的 hover 翻译功能不受影响）
...
1.  **添加邮箱**：
    ...
    *   授权成功后关闭授权窗口，邮箱自动加入列表，Toast 提示"邮箱连接成功"

    *   用户拒绝或关闭授权窗口：Toast 提示"授权已取消"

    *   授权失败：Toast 提示"授权失败，请重试"

    *   同一个 Gmail 邮箱地址不可重复添加，若检测到已存在，Toast 提示"该邮箱已添加，请勿重复操作"
...
2.  **附件显示**：
    ...
    *   附件提取规则：附件信息来源为邮件 API 返回的 attachment 数据结构（如 Gmail API 的 parts.body.attachmentId），并非从 HTML 中解析。在原型中采用 HTML 结构匹配作为模拟方案
...
4.  **引用内容处理**：
    ...
    *   点击「...」按钮展开全部引用内容，引用内容以视觉区分样式展示，与正文内容明显区别，具体样式以设计稿为准
...
6.  **文本限制**：
    ...
    *   正文最多 2000 字符，超出阻止输入（无 Toast 提示）。字符数统计纯文本内容，不含 HTML 标签，图片 / 附件不占字符数。
...
4.  **会话重新激活分配**：
    ...
    *   结束的邮件会话收到新邮件时，��话重新激活并分配给原服务客服，不重新进入排队分配流程

    *   原服务客服不可用时（离职/被禁用），按照 Web 会话的会话负责人不可用逻辑处理
