...
* 系统自动过滤 no-reply 类邮件地址（noreply@、no-reply@、no_reply@、donotreply@、do-not-reply@、do_not_reply@、mailer-daemon@，不区分大小写），此类邮件不进入消息队列
...
| US-05 | 管理员 | 我希望在档案中按来源渠道筛选邮件会话 | 会话记录列表增加"来源渠道"筛选项和列，支持 Web / 网页插件 / Email 筛选 | P1 |
...
| US-08 | 客服 | 我希望在访客/客户列表中向访客发送邮件 | 访客/客户列表的操作菜单显示"发送邮件"，点击打开邮件编辑弹窗，发送后跳转到消息 | P1 |
| US-09 | 客服 | 我希望在档案中快速进入或加入已结束的 email 会话 | 若当前客服是服务客服显示"进入会话"，否则显示"加入会话"，点击后跳转到消息对应分类下消息 | P1 |
...
graph LR
  ...
  D[档案] --> D1[会话记录]
  D1 --> D1a[来源渠道筛选: Web / 网页插件 / Email]

  E[访客] --> E1[在线访客]
  E[访客] --> E2[全部访客]
  E2 --> E2a[来源渠道筛选: Web / 网页插件 / Email]
...
flowchart TD
  ...
  E --> F[弹出添加邮箱弹窗]
  F --> G[选择 Gmail]
  G --> H[新窗口打开 Google OAuth 授权]
  H --> I{授权结果}
  I -- 成功 --> J[关闭授权窗口，邮箱出现在列表 + Toast: 邮箱连接成功]
  I -- 取消/关闭 --> J2[关闭授权窗口 + Toast: 授权已取消]
  I -- 失败 --> J3[关闭授权窗口 + Toast: 授权失败，请重试]
...
2. 若已有邮箱，以表格形式展示，列为：邮箱、状态、创建人、创建时间、操作
...
1. **添加邮箱**：
    ...
    * 点击 Gmail 卡片后在新窗口打开 Google OAuth 授权页面

    * 授权成功后关闭授权窗口，回跳Email列表页面，邮箱自动加入列表，Toast 提示"邮箱连接成功"

    * 用户拒绝或关闭授权窗口：回跳Email列表页面，Toast 提示"授权已取消"

    * 授权失败：回跳Email列表页面，Toast 提示"授权失败，请重试"

    * 同一个 Gmail 邮箱地址不可重复添加，若检测到已存在，回跳Email列表页面，Toast 提示"该邮箱已添加，请勿重复操作"

    * 邮箱数量上限 99 个，达到上限时点击「添加」按钮toast提示"最多添加 99 个邮箱"
...
2. **删除邮箱**：
    ...
    * Token 撤销失败时 Toast 提示"操作失败，请重试"，本地不删除
...
3. **Token 失效处理**：
    ...
    * 系统检测到 token 失效时，邮箱列表中该邮箱状态标记为"失效"
...
4. **表格字段**：
    ...
    * 操作：正常状态的邮箱显示「删除」；失效状态的邮箱显示「重新连接」、「删除」

5. **重新连接**：

    * 当邮箱状态为"失效"时，操作列下拉面板中显示"重新连接"选项

    * 点击"重新连接"后在新窗口打开 Google OAuth 授权页面，流程与添加邮箱一致

    * 授权成功：关闭授权窗口，邮箱状态从"失效"恢复为"正常"，Toast 提示"邮箱连接成功"

    * 用户拒绝或关闭授权窗口：Toast 提示"授权已取消"，邮箱状态保持"失效"不变

    * 授权失败：Toast 提示"授权失败，请重试"，邮箱状态保持"失效"不变

    * 重新连接不会改变邮箱的创建时间、创建人等信息，仅刷新 OAuth token

    * 重新连接成功后，该邮箱关联的进行中会话恢复正常收发能力
...
**用户场景**：客服打开工作台后，需要快速识别哪些是邮件会话、哪些是 Web 会话，哪些是网页插件会话， 哪些是 Email 会话。
...
3. **会话标题**：
    ...
    * Email 会话标题按以下优先级展示：1. 邮件 subject 字段（非空时）→ 2. 访客备注名（已设置时）→ 3. 访客姓名 → 4. 发件人邮箱地址（兜底）
...
2. **附件显示**：
    ...
    * 附件提取规则：附件信息来源为邮件 API 返回的 attachment 数据结构（如 Gmail API 的 parts.body.attachmentId），并非从 HTML 中解析。

    * 附件区域位于邮件正文下方

    * 附件展示格式：附件格式缩略图 + 附件名称 + 附件大小。缩略图沿用网页会话文件缩略图

    * 点击附件在弹窗中打开预览，沿用原会话附件预览逻辑。

3. **空邮件正文**：

    * 邮件正文为空时，消息气泡显示占位文案"（无邮件内容）"

4. **消息操作**：

    * Email 会话中的消息仅显示翻译操作（不显示回复、复制、撤回等）

    * 仅翻译文本内容，图片、附件不翻译。译文需支持 HTML 格式。
...
4. **引用内容处理**：
    ...
    * 点击「...」按钮展开全部引用内容，引用内容以视觉区分样式展示，与正文内容明显区别，具体样式以设计稿为准。打开状态不记忆，重新进入会话收起所有展开的引用内容。
...
3. **工具栏**：
    ...
    * 插入按钮：超链接——点击后弹出链接输入面板，URL 最多 500 字符，超出阻止输入。输入框失焦校验：为空时提示"请输入URL"，格式不合法时提示"无效URL"。插入按钮在输入为空时置灰
...
6. **文本限制**：
    ...
    * 正文最多 2000 字符，超出阻止输入。字符数统计纯文本内容，不含 HTML 标签，图片 / 附件不占字符数。
...
7. **发送按钮**：
    ...
    * 发送失败：消息气泡显示发送失败状态（红色感叹号），支持点击重新发送
...
3. **结束会话弹窗**：
    ...
    * 邮件会话结束并不会像 Web 渠道会话真正结束，邮件会话结束后可重新开启会话。邮件会话结束后底部输入框区域显示「重新开启」按钮，点击后会话恢复开启状态，底部恢复显示邮件编辑器，Toast 提示"会话已重新开启"

4. **按钮禁用**：

    * 会话已关闭时，会话头部所有操作按钮置灰不可用
...
**功能描述**：会话记录档案中增加"来源渠道"维度，支持按 Web/网页插件/Email 筛选和展示，并提供不同身份下的差异化操作。
...
1. **筛选项**：
    ...
    * 新增"来源渠道"下拉筛选，选项：Web / 网页插件 / Email
...
3. 填写邮件主题（必填，最多 200 字符，超出阻止输入）

4. 在富文本编辑区域编写邮件内容，最多 2000 字符。富文本支持加粗、斜体、下划线、有序/无序列表、超链接、附件、图片

4. 点击"确认发送"发送邮件

5. 发送成功后弹窗关闭，Toast 提示"发送成功"，自动跳转到消息列表并定位到该邮件会话
...
2. **弹窗规则**：
    ...
    * 弹窗中增加「主题」输入字段，位于 To/From 字段与编辑器之间，必填，最多 200 字符，超出阻止输入

    * 编辑区域为富文本输入（contenteditable），最多 2000 字符

    * 超出字符上限时阻止输入

    * 主题为空或编辑区域为空时"确认发送"按钮置灰
...
2. 弹出发送邮件弹窗，标题"发送邮件"，下方显示"发送给: {客户名称}"
...
1. **操作菜单条件**：
    ...
    * 客户列表：操作菜单显示"创建会话"、"发起聊天"和"发送邮件"。若客户邮箱为空，点击"发送邮件"toast提示：客户邮箱为空，无法发送。若未配置任何邮箱渠道，点击"发送邮件"toast提示：未配置邮箱渠道，无法发送。
...
1. **结束 email 会话**：
    ...
    * 客服可在已结束的邮件会话底部点击「重新开启」按钮，会话恢复到开启状态

    * 重新开启后底部恢复显示邮件编辑器，头部操作按钮恢复可用，Toast 提示“会话已重新开启”
...
4. **会话重新激活分配**：
    ...
    * 原会话负责人不可用时（删除/被禁用），按照 Web 会话的会话负责人不可用逻辑处理
...
5. **邮件线程匹配**：
    ...
    * 同一封邮件（相同 Message-ID）发送到多个已连接邮箱时只创建一个会话，关联到第一个匹配的收件邮箱
...
7. **新邮件通知**：
    ...
    * 新邮件会话分配给客服后的通知行为与 Web 新会话一致（浏览器 Push 通知）
...
1. **访客列表**：
    ...
    * 新增"来源渠道"筛选项，下拉选项：Web / 网页插件 / Email
...
1. **会话列表**：
    ...
    * 新增"来源渠道"表头列和筛选项，下拉选项：Web / 网页插件 / Email
