...
| Key Result | 量化标准 |
|-----------|---------|
| KR1：平台覆盖 | 支持Slack、飞书、钉钉、企业微信、自定义 5 种渠道 |
| KR2：安全保障 | 所有推送消息支持签名验证，防止伪造和篡改 |
...
| US-04 | 管理员 | 我希望删除不再使用的 Webhook 配置，以便保持列表整洁 | 删除前有二次确认，确认后从列表移除并停止推送 | P1 |
| US-05 | 开发者 | 我希望使用自定义 Webhook 对接内部系统，以便将客服事件接入自建工作流 | 选择自定义渠道后填写任意 HTTPS 地址 | P1 |
...
1. **列表字段**：
   - 名称：Webhook 的配置名称
   - 渠道：所属平台名称（枚举值：飞书/钉钉/Slack/企业微信/自定义）
   - 状态：启用/禁用开关
   - 创建时间：格式为 YYYY-MM-DD
   - 创建人：显示头像 + 昵称
   - 操作：编辑、删除
...
关键字段的 label 右侧显示问号图标（?），鼠标 hover 时显示气泡提示：
...
| 点击保存 | URL 唯一性 | 重复 Webhooks URL | Toast 提示 |
...
**弹窗交互**：
1. 点击弹窗左上角「返回」按钮返回选择渠道弹窗
2. 点击弹窗左上角「关闭」按钮返回选择渠道弹窗
3. 返回选择渠道弹窗后已输入的内容清空，不保存
...
| 添加 Webhook | 可用 | 弹出升级提示 | 弹出升级提示 |
| 编辑 Webhook | 可用 | 弹出升级提示 | 弹出升级提示 |
| 删除 Webhook | 可用 | 弹出升级提示 | 弹出升级提示 |
| 启用/禁用 Webhook | 可用 | 弹出升级提示 | 弹出升级提示 |
| 启用事件提醒 | 可用 | 弹出升级提示 | 弹出升级提示 |

免费版/服务过期用户可进入 Webhooks 页面查看内容，但所有写操作（添加/编辑/删除/启用）均会弹出升级提示弹窗。弹窗行为遵循全局服务版本规则
...
| Webhook URL 重复 | 阻止保存操作 | 提示「重复 Webhook URL」 |
