...
|------|------------|---------|-------------|
| 飞书 | 签名密钥 | **必填** | 在飞书群设置中添加自定义机器人，复制 Webhook 地址 |
| 钉钉 | 加签密钥 | **必填** | 在钉钉群设置中添加自定义机器人，复制 Webhook 地址 |
| Slack | 签名密钥 | **必填** | 在 Slack App 中启用 Incoming Webhooks，复制 Webhook URL |
| 企业微信 | 签名密钥 | **必填** | 在企业微信群中添加群机器人，复制 Webhook 地址 |
| 自定义 | - | 无需填写 | 输入你的自定义 Webhook 接收地址 |

**保存校验**：
- 配置名称和 Webhook URL 必填
- 飞书、钉钉、Slack、企业微信渠道的签名密钥必填
- 展示字段至少勾选一项
- Webhook URL 不能与已有配置重复
