...
| 不确定某个会话属于哪个分类时，需要逐一翻找 | 增加查找时间，可能导致会话遗漏 |
...
| KR2：降低会话查找成本 | 在「全部」分类下可通过搜索和筛选定位任意会话 |

### 1.3 非目标（本期不做）

- 「全部」分类下按状态分组展示
- 「全部」分类的未读数角标
- 可组合筛选重构（将状态从导航改为筛选条件）
...
| 实体名 | 字段 | 类型 | 说明 |
|--------|------|------|------|
| 分类项 | 标识 | 字符串 | 「全部」分类的系统标识，区别于访客/客户筛选中的「全部」 |
| 分类项 | 名称 | 字符串 | 显示文案：「全部」 |
| 在线会话 | 状态 | 枚举 | 待回复、排队中、待处理、已回复 |
...
| 功能 | 客服 | 说明 |
|------|------|------|
| 查看「全部」分类 | 可见 | 与其他分类权限一致 |
| 在「全部」下执行操作 | 与各分类一致 | 操作权限不因分类变化而改变 |
...
| 异常场景 | 处理方式 | 用户感知 |
|---------|---------|---------|
| 「全部」分类下无会话 | 展示空状态提示 | 显示「暂无符合条件的会话」 |
