...
1.  **编辑快捷入口时**：当已关联的常见问题被删除后，下次打开编辑弹窗时，系统检测到所选问题已不存在，在下拉选择器下方显示行内警告「所选常见问题已被删除，请重新选择」。点击确定时阻止提交，toast 提示「所选常见问题已被删除，请重新选择」，同时清空选择。
    
2.  **访客端点击时**：当访客点击关联了已删除常见问题的快捷入口时，不发送消息，toast 提示「该快捷入口暂时不可用」。

3.  **管理端失效标记**：当快捷入口关联的常见问题被删除后，管理端快捷入口列表中该条目显示警告标记（感叹号三角图标，黄色），鼠标悬停时显示 tooltip「关联的常见问题已被删除，请重新配置」，直到管理员重新编辑并保存有效内容后标记消除。