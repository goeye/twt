# Deploy web-agent & web-customer & web-admin

同时构建并部署客服端（web-agent）、访客端（web-customer）和运营后台（web-admin）到预览服务器。

## 步骤

1. 并行运行 `pnpm deploy:web-agent`、`pnpm deploy:web-customer` 和 `pnpm deploy:web-admin`（各自会先 build 再 rsync 到远程服务器）
2. 等待所有命令执行完成
